...
import { SelectIntentDialogStyled } from './index.style';
  ...
  return (
    <Dialog open={open} fullWidth disableBackdropClick>
      <SelectIntentDialogStyled>
        <DialogTitle>{t('confirmClientMsg')}</DialogTitle>
        <DialogContent>
          <DialogContentText>
          ...
          <Box>
            <Autocomplete
              ...
              multiple
              options={
                (message && message.intent && message.intent.slots) || []
              }
              getOptionLabel={(option) => option.name}
              onChange={(e, newValue) => {
